...
from moviepy.editor import VideoClip, ColorClip, ImageClip, concatenate_videoclips, AudioFileClip
...
FPS   = 30
DUR   = 24                 # seconds for chart animation
...
if not ELEVEN_API_KEY:
    logging.warning("⚠️  Warning: ELEVENLABS_API_KEY not set in environment; TTS will be disabled.")

if not DEFAULT_BUCKET:
    logging.warning("⚠️  Warning: GCS_BUCKET not set in environment; output_uri must be provided.")
...
def load_df(data: List[Dict[str, Any]]) -> pd.DataFrame:
    # Expect fields: date (str/datetime), close (float)
...
def render_chart_frame(df: pd.DataFrame, u: float, video_title: str, subtitle: str = "") -> np.ndarray:
    ...
    # Set sparse x labels (6 ticks max)
    if len(x) > 1:
    ...
    # Blend with gradient (simple lighten)
    ...
    # Glass card with live price and animated % change
...
def make_chart_clip(df: pd.DataFrame, video_title: str, subtitle: str, duration=DUR) -> VideoClip:
...
def render_reel(cloud_event):
    ...
    Expects JSON payload in message.data with fields:
      ...
      - duration (int, optional; defaults to DUR)
    """
    try:
    ...
    duration: int = int(payload.get("duration", DUR))
    ...
    if tts_text:
        ...
        duration = tts_duration  # stretch chart to speech length

    # Build clips
    intro = make_slate(title, subtitle or "Previous trading hours", seconds=0.8)
    chart = make_chart_clip(df, video_title=title, subtitle=subtitle, duration=duration)
    outro = make_slate("Follow for daily updates", "", seconds=0.8)

    final = concatenate_videoclips([intro, chart, outro], method="compose").fadein(0.25).fadeout(0.25)

    # Attach narration if available
    if audio_path:
        narr = AudioFileClip(str(audio_path))
        final = final.set_audio(narr.set_duration(final.duration))

    # Write video to /tmp then upload to GCS
    ...
    try:
        ...
        if audio_path: Path(audio_path).unlink(missing_ok=True)
    except Exception:
        pass

    # Cloud Functions don't need to return a body for Pub/Sub events; log is enough
    return
